...
def detect_lines(image: np.ndarray) -> np.ndarray:
    ...
    """
    edges = cv2.Canny(image, MIN_CANNY_THRESHOLD, MAX_CANNY_THRESHOLD, apertureSize=3)
    line_pts = cv2.HoughLinesP(edges, rho=1, theta=np.pi/180, threshold=100, minLineLength=MIN_LINE_LENGTH, maxLineGap=MAX_LINE_GAP)
    """ END YOUR CODE HERE """
...
def get_pairwise_intersections(lines: np.ndarray) -> np.ndarray:
    ...
    for i in range(num_lines):
        for j in range(i+1, num_lines):
            ...
            if point[2] == 0:
                continue
            intersections[count] = point / point[2]
            count += 1
...
def get_support_mtx(intersections: np.ndarray, lines: np.ndarray, distance_threshold: float = DISTANCE_THRESHOLD):
    ...
    for i in range(num_intersections):
        for j in range(num_lines):
            support_mtx[i][j] = 1.0 if (np.abs(np.dot(lines[j], intersections[i])) < distance_threshold * np.sqrt(np.dot(lines[j][:2], lines[j][:2]))) else 0.0
    """ END YOUR CODE HERE """
...
def get_vanishing_pts(lines: np.ndarray, num_vanishing_pts: int) -> np.ndarray:
    ...
    for i in range(num_vanishing_pts):
        num_support = np.sum(support_mtx, axis=1)
        idx = np.argmax(num_support)
        vanishing_pts.append(intersections[idx])
        mask = support_mtx[idx] == 1
        support_mtx[:, mask] = 0
    """ END YOUR CODE HERE """
...
def get_target_height(vanishing_line: np.ndarray, query_info: dict, target_info: dict, vanishing_pt_v: np.ndarray):
    ...
    print(vanishing_line, vanishing_pt_v)
    target_height = 0.
    """ 
    YOUR CODE HERE
    ...
    """
    # line_1 = np.cross(pt_t1, pt_b1)
    line_2 = np.cross(pt_t2, pt_b2)
    u_vanishing_pt = np.cross(np.cross(pt_b1, pt_b2), vanishing_line)
    u_vanishing_pt /= u_vanishing_pt[2]
    pt_t1_tilde = np.cross(np.cross(pt_t1, u_vanishing_pt), line_2)
    pt_t1_tilde /= pt_t1_tilde[2]
    dv = np.linalg.norm(pt_b2 - vanishing_pt_v)
    dt1_tilde = np.linalg.norm(pt_b2 - pt_t1_tilde)
    dt2 = np.linalg.norm(pt_b2 - pt_t2)
    target_height = query_d1 * (dt2 * (dv - dt1_tilde)) / (dt1_tilde * (dv - dt2))
    """ END YOUR CODE HERE """
