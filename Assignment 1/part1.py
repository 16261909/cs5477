...
def _visiual_lines_in_image(src_img:np.ndarray,lines_vec: list):
    for i in range(0, len(lines_vec)):
        cv2.line(src_img, lines_vec[i].point1.coordinate, lines_vec[i].point2.coordinate, (0, 255, 255), 5)
...
def _confirm_select_constraints(str=''):
    ...
    root = tkinter.Tk()
    root.withdraw()
    ans=tkinter.messagebox.askokcancel('Note', 'Do you wish to select '+str+' lines on the shown image?')
    return ans
...
def _affinely_rectified_test(image,image_name):
    ...
    para_lines_vec = _get_points_from_files(save_file=os.path.join(DATA_DIR, image_name+'_Parallel_lines.csv'))
    _visiual_lines_in_image(src_img=src_image, lines_vec=para_lines_vec)
    # compute affine rectification
    affined_image = compute_affine_rectification(src_img=src_image, lines_vec=para_lines_vec)
    # visualization
...
def _metric_rectified_twostep_test(image,image_name):
    ...
    # read points (x1,y1) & (x2,y2) from files to form their line vectors (l1,l2,l3)
    ortho_2_lines_vec = _get_points_from_files(save_file=os.path.join(DATA_DIR,image_name+'_Orthogonal_2_lines.csv'))
    _visiual_lines_in_image(src_img=affinely_rectified_image, lines_vec=ortho_2_lines_vec)
    # compute metric rectification
    rectified_image1 = compute_metric_rectification_step2(src_img=affinely_rectified_image, line_vecs=ortho_2_lines_vec)
    # visualization
...
def _metric_rectified_onestep_test(image,image_name):
    ...
    # read points (x1,y1) & (x2,y2) from files to form their line vectors (l1,l2,l3)
    ortho_5_lines_vec = _get_points_from_files(save_file=os.path.join(DATA_DIR, image_name+'_Orthogonal_5_lines.csv'))
    _visiual_lines_in_image(src_img=src_image,lines_vec=ortho_5_lines_vec)
    # compute metric rectification
    rectified_image2 = compute_metric_rectification_one_step(src_img=src_image, line_vecs=ortho_5_lines_vec)
    # visualization
